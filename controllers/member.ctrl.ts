...
async function add(req: NextApiRequest, res: NextApiResponse) {
  ...
  const addResult = await MemberModel.add({ displayName, email, photoURL, uid });
...
async function findByScreenName(req: NextApiRequest, res: NextApiResponse) {
  ...
  const extraScreenName = Array.isArray(screenName) ? screenName[0] : screenName;
  const findResult = await MemberModel.findByScreenName(extraScreenName);
  if (findResult === null) {
    return res.status(404).end();
  }
  res.status(200).json(findResult);
}
const MemberCtrl = { add, findByScreenName };
